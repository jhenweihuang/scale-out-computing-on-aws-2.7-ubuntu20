...
class ManageImage(Resource):
    ...
    def post(self):
        ...
        if args["os"].lower() not in ["centos7", "rhel7", "amazonlinux2", "windows", "ubuntu20"]:
            return errors.all_errors('CLIENT_MISSING_PARAMETER', "os must be centos7, rhel7, amazonlinux2, ubuntu20 or windows")
        ...
        if ami_label not in soca_labels.keys():
            try:
                ...
                if (len(ec2_response["Images"]) != 0):
                    ...
                    return errors.all_errors('IMAGE_REGISTER_ERROR', f"{ami_id} is not available in AWS account. If you just created it, make sure the stateof the image is 'available' on the AWS console")
